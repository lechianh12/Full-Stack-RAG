...
from module.session_schema import (
    SessionRequest,
)  # <-- Sửa tên (nếu file session_schema.py của bạn tên là SessionRequest)
    ...
    for file in files:
        try:
            ...
            if make_global:
                ...
                logger.info(
                    f"Admin {current_user.username} uploaded GLOBAL document: {file.filename}"
                )
            else:
            ...
            logger.info(
                f"File {file.filename} ingested successfully into collection {collection_name}"
            )
...
def query_data(
    query_data: QueryData, current_user: Authen = Depends(get_current_user)
):  # Sửa dependency
